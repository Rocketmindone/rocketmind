...
> Вариант (`btn-primary` и т.д.) и размер — ортогональные оси: вариант определяет стиль, размер — контекст (hero → lg, форма → md, compact UI → sm). Размеры шрифта привязаны к стилям `label-12/14/16` из раздела 2.2 Типографика.
